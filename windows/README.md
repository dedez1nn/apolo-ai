...
interface de revisão (`apolo review`) na própria janela do app (sem console
nenhum no meio — é um app desktop, Flet), e sobe um ícone na bandeja do
sistema que fica disponível depois pra reabrir a revisão ou ligar o Proton
Bridge. Não faz parte do núcleo do Apolo — é puramente um lançador.
...
Duplo clique no atalho (ou no `.exe` direto): abre **na hora** a janela de
`apolo review` e sobe o ícone na bandeja do sistema (visível ou escondido
atrás da seta `^`, perto do relógio). Fechar a janela de revisão não encerra
o Apolo — o ícone continua ali, e pelo menu dele dá pra:
- "Abrir revisão" — reabre a janela com `apolo review` (é o item padrão:
...
o Flet instalado a UI não abre (rode o setup do `../README.md` primeiro).
